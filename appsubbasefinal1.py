...
st.markdown('<div class="sub-header">Predict resilient modulus for subbase layer using machine learning model</div>', unsafe_allow_html=True)
...
# Input fields
layer_thickness = st.number_input("Layer Thickness (inches)", min_value=0.0, value=150.0, step=10.0, help="Enter the thickness of the subbase layer")
...
aashto_options = ["A-1-a", "A-1-b", "A-2-4", "A-2-5", "A-2-6", "A-2-7", "A-3", "A-4", "A-5", "A-6", "A-7-5", "A-7-6"]
...
aashto_encoding = {
    "A-1-a": 0, "A-1-b": 1, "A-2-4": 2, "A-2-5": 3, 
    "A-2-6": 4, "A-2-7": 5, "A-3": 6, "A-4": 7, 
    "A-5": 8, "A-6": 9, "A-7-5": 10, "A-7-6": 11
}
...
max_lab_dry_density = st.number_input("Maximum Lab Dry Density (pcf)", min_value=1.0, max_value=3.0, value=2.0, step=0.01, help="Maximum dry density from laboratory compaction test")
    ...
    if st.button("🔮 Predict Resilient Modulus", type="primary", use_container_width=True):
        if subbase_model is not None:
            try:
                ...
                st.metric(label="Predicted Resilient Modulus", value=f"{prediction:.2f} ksi")
